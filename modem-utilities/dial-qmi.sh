...
nas_path="$path/temp-nas_$INTERFACE"
...
${SUDO} qmicli -p -d $wdm --client-no-release-cid --nas-noop | ${SUDO} tee  $nas_path
while [ ! -s "$nas_path" ]
do
	echo "re-allocate the nas resource"
	sleep 0.5
	${SUDO} qmicli -p -d $wdm --client-no-release-cid --nas-noop | ${SUDO} tee  $nas_path
done
echo "Allocate nas resource succcess"
